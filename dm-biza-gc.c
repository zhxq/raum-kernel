	...
	for (src_offset = 0; src_offset < bt->params->zone_capacity_chunk; ++src_offset)
	{
		...
		if (biza_map_is_data_in_pcn_useful(bt, src_pcn))
		{
			...
			if (dst_zone->wp >= dst_zone->start + dst_zone->capacity)
			{
				...
				if (dst_dev->open_zones[dst_oz_idx] == dst_dev->nr_zones)
					BUG_ON(1);
...
static void biza_gc_work(struct work_struct *work)
{
	...
	{
		pr_err("user_send %lld, ssd user_read %lld bytes/%lld requests, data write %lld, gc write %lld, num chunks %lld, parity write %lld, parity oop write %lld, data in place update %lld, parity in place upate %lld\n",
			   atomic64_read(&bt->user_send), atomic64_read(&bt->user_read), atomic64_read(&bt->user_read_reqs), atomic64_read(&bt->data_write), atomic64_read(&bt->gc_write),
			   atomic64_read(&bt->num_chunks), atomic64_read(&bt->parity_write), atomic64_read(&bt->oop_parity_write),
	...
	if (biza_target_idle(bt) || bt->gc->p_free_zones < bt->gc_limit_low)
	{
		...
		gc->kc_throttle.throttle = BIZA_GC_LOWEST_SPEED +
