...
static void biza_gc_work(struct work_struct *work)
{
    ...
    {
        pr_err("user_send %lld, data write %lld, parity write %lld, parity oop write %lld, data in place update %lld, parity in place upate %lld\n",
               atomic64_read(&bt->user_send), atomic64_read(&bt->data_write), atomic64_read(&bt->parity_write), atomic64_read(&bt->oop_parity_write),
