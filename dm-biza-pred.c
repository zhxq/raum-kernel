...
static inline void biza_heap_entry_swap(struct biza_heap_entry **a,
					struct biza_heap_entry **b)
{
...
static void heapifyUp(struct biza_heap *queue, int index)
{
	...
	while (index > 0 && queue->elements[index]->priority >
				    queue->elements[parent]->priority) {
		biza_heap_entry_swap(&queue->elements[index],
				     &queue->elements[parent]);
		index = parent;
		parent = (index - 1) / 2;
	}
}
...
static void heapifyDown(struct biza_heap *queue, int index)
{
	...
	if (left < queue->size && queue->elements[left]->priority >
					  queue->elements[largest]->priority) {
		largest = left;
	}

	if (right < queue->size && queue->elements[right]->priority >
					   queue->elements[largest]->priority) {
		largest = right;
	}
...
static void biza_heap_enqueue(struct biza_heap *queue,
			      struct biza_heap_entry *element)
{
...
static struct biza_heap *biza_heap_create(int capacity)
{
	...
	heap->elements = kzalloc(capacity * sizeof(struct biza_heap_entry *),
				 GFP_KERNEL);
	heap->size = 0;
...
static void printHeap(struct biza_heap *queue)
{
	...
	for (i = 0; i < queue->size; i++) {
		pr_err("(%d, %d) ", queue->elements[i]->key,
...
inline static struct biza_htable_entry *biza_htable_find(struct biza_target *bt,
							 sector_t key)
{
...
inline static void biza_update_reuse_dist(struct biza_target *bt,
					  struct biza_pred_entry *etr,
					  uint32_t new_dist)
{
	...
	if (etr->aware_type == BIZA_ZRWA_AWARE) {
		...
		if (etr->reuse_dist > bt->max_dist_lfta_set)
			bt->max_dist_lfta_set = etr->reuse_dist;
		if (etr->reuse_dist < bt->min_dist_lfta_set)
			bt->min_dist_lfta_set = etr->reuse_dist;
	}
}

inline static void biza_update_reuse_cnt(struct biza_target *bt,
					 struct biza_pred_entry *etr,
					 uint32_t new_cnt)
{
	...
	if (etr->aware_type == BIZA_ZRWA_AWARE ||
	    etr->aware_type == BIZA_LIFETIME_AWARE) {
...
inline static bool
biza_can_insert_zrwa_aware_set(struct biza_target *bt,
			       struct biza_pred_entry *pred_etr)
{
	struct biza_heap_entry *n = NULL;
	uint32_t threshold;

	if (pred_etr->aware_type != BIZA_LIFETIME_AWARE)
		return false;
	if (pred_etr->reuse_dist > BIZA_ZRWA_AWARE_REUSE_DIST_THRESHOLD)
		return false;
	if (!biza_heap_is_full(bt->heap_dist))
		return true;
...
static void biza_insert_zrwa_aware_set(struct biza_target *bt,
				       struct biza_pred_entry *new_etr)
{
	struct biza_heap_entry *new_heap_etr =
		kzalloc(sizeof(struct biza_heap_entry), GFP_KERNEL);

	if (biza_heap_is_full(bt->heap_dist))
		biza_evict_zrwa_aware_set(bt);
...
inline static bool
biza_can_insert_lifetime_aware_set(struct biza_target *bt,
				   struct biza_pred_entry *pred_etr)
{
	struct biza_heap_entry *n = NULL;
	uint32_t threshold;
	...
	if (pred_etr->aware_type != BIZA_TRIVIAL)
		return false;
	if (pred_etr->reuse_cnt < BIZA_LIFETIME_AWARE_REUSE_CNT_THRESHOLD)
		return false;
	if (!biza_heap_is_full(bt->heap_cnt))
		return true;
...
inline static void biza_evict_lifetime_aware_set(struct biza_target *bt)
{
	...
	BUG_ON(del_pred_etr->aware_type != BIZA_ZRWA_AWARE &&
	       del_pred_etr->aware_type != BIZA_LIFETIME_AWARE);
...
static void biza_insert_lifetime_aware_set(struct biza_target *bt,
					   struct biza_pred_entry *new_etr)
{
	struct biza_heap_entry *new_heap_etr =
		kzalloc(sizeof(struct biza_heap_entry), GFP_KERNEL);
	...
	if (biza_heap_is_full(bt->heap_cnt))
		biza_evict_lifetime_aware_set(bt);
...
static void biza_evict_pred_set(struct biza_target *bt)
{
	...
	while (true) {
		del_pred_etr = list_first_entry(
			&bt->lru->list, struct biza_pred_entry, lru_link);
		// pr_err("del_pred_etr: 0x%px\n", del_pred_etr);
		if (del_pred_etr->reuse_cnt >= 2) {
			list_del(&del_pred_etr->lru_link);
			biza_update_reuse_cnt(bt, del_pred_etr,
					      del_pred_etr->reuse_cnt - 2);
	...
	if (del_tb_etr && (&del_tb_etr->link)) {
		hash_del(&del_tb_etr->link);
	}
	// pr_err("freeing del_pred_etr: 0x%px\n", del_pred_etr);
	kfree(del_pred_etr);
	kfree(del_tb_etr);
}
...
static void biza_insert_pred_set(struct biza_target *bt, sector_t lcn)
{
	...
	if (bt->lru->size >= BIZA_PRED_SET_CAPACITY)
		biza_evict_pred_set(bt);
...
inline static void biza_update_entry(struct biza_target *bt,
				     struct biza_pred_entry *pred_etr,
				     sector_t lcn)
{
	biza_update_reuse_cnt(bt, pred_etr, pred_etr->reuse_cnt + 1);
	biza_update_reuse_dist(bt, pred_etr,
			       bt->wrt_time - pred_etr->last_wrt_time);
	pred_etr->last_wrt_time = bt->wrt_time;
...
inline static void biza_promote_entry(struct biza_target *bt,
				      struct biza_pred_entry *pred_etr)
{
				       ...
				       uint8_t drive_idx, uint32_t hint)
{
	...
	case BIZA_TRIVIAL:
		ozg_idx = (dev->nr_zrwa_aware_open_zones +
			   ...
			   get_random_u32() % dev->nr_trivial_open_zones) /
			  BIZA_NR_ISOLATION_DOMAIN;
		oz_idx = biza_get_oz_idx_gc_avoid(bt, dev, ozg_idx);
		break;
	default:
