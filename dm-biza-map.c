...
inline bool biza_map_is_data_in_pcn_useful(struct biza_target *bt, sector_t pcn)
{
	...
	if (bt->map->p2l[pcn].chunk_no == BIZA_MAP_UNMAPPED || bt->map->p2l[pcn].chunk_no == BIZA_MAP_INVALID)
	{
		 ...
		 */
		if (bt->map->p2l[pcn].stripe_no == BIZA_MAP_UNMAPPED || bt->map->p2l[pcn].stripe_no == BIZA_MAP_INVALID)
			ret = false;
		else
		{
		...
		if (--org_stripe->valid == 0)
		{
			if (org_stripe->used == bt->params->k)
			{
				for (i = 0; i < bt->params->m; ++i)
				{
					...
					if (org_parity_pcn >= bt->params->nr_internal_chunks)
						continue;
				...
				xa_erase_irq(&bt->map->stripe_table, org_stripe_no);
...
void biza_map_update_parity_wrt(struct biza_target *bt, sector_t pcn, uint64_t no, uint8_t slot)
{
	...
	else
	{
		...
		if (old_pcn != BIZA_MAP_INVALID && old_pcn != BIZA_MAP_UNMAPPED)
		{
...
void biza_map_remap(struct biza_target *bt, sector_t src_pcn, sector_t dst_pcn)
{
	...
	{ // parity chunk
		...
		stripe = xa_load(&bt->map->stripe_table, stripe_no);
		// pr_warn("GC parity stripe 0x%llx, src_pcn 0x%llx, dst_pcn 0x%llx\n", stripe_no, src_pcn, dst_pcn);
		...
		else
		{
			if (stripe->parity_pcns[slot] != src_pcn)
			{
				...
				biza_pcn_to_idx(bt, src_pcn, &passed_drive_idx, &passed_zone_idx, &passed_offset);
				pr_err("read: src_pcn: 0x%llx, drive_idx: %u, zone_idx: %u, offset: 0x%llx; passed: src_pcn: 0x%llx, drive_idx: %u, zone_idx: %u, offset: 0x%llx\n",
					   stripe->parity_pcns[slot], read_drive_idx, read_zone_idx, read_offset, src_pcn, passed_drive_idx, passed_zone_idx, passed_offset);
			}
	...
	{ // data chunk
		...
		pr_warn("GC parity lcn 0x%llx, src_pcn 0x%llx, dst_pcn 0x%llx\n", lcn, src_pcn, dst_pcn);
		if (bt->map->l2p[lcn].chunk_no != src_pcn)
		{
			pr_err("mismatch src_pcn -- lcn: 0x%llx, src pcn: 0x%llx, current lcn -> pcn: 0x%llx, dst pcn: 0x%llx, stripe_no: 0x%llx, slot: 0x%x\n", lcn, src_pcn, stripe->parity_pcns[slot], dst_pcn, stripe_no, slot);
			biza_pcn_to_idx(bt, bt->map->l2p[lcn].chunk_no, &read_drive_idx, &read_zone_idx, &read_offset);
			biza_pcn_to_idx(bt, src_pcn, &passed_drive_idx, &passed_zone_idx, &passed_offset);
			pr_err("read: src_pcn: 0x%llx, drive_idx: %u, zone_idx: %u, offset: 0x%llx; passed: src_pcn: 0x%llx, drive_idx: %u, zone_idx: %u, offset: 0x%llx\n",
				   bt->map->l2p[lcn].chunk_no, read_drive_idx, read_zone_idx, read_offset, src_pcn, passed_drive_idx, passed_zone_idx, passed_offset);
		}
...
int biza_ctr_map(struct biza_target *bt)
{
	...
	bt->map->p2l = vmalloc(bt->params->nr_internal_chunks * sizeof(biza_addr_t));
	if (!bt->map->p2l)
	{
